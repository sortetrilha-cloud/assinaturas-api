...
IMAGEM_PADRAO = "app/static/imagens/logo_email.png"
...
def enviar_email_loteria(destinatario, jogos: pd.DataFrame, loteria, produto):
    service = autenticar_gmail()

    # Lê o HTML do template
    with open("app/templates/email_template.html", "r", encoding="utf-8") as f:
        corpo_html = f.read()
        corpo_html = corpo_html.replace("{{PRODUTO}}", produto)
        corpo_html = corpo_html.replace("{{LOTERIA}}", loteria)
        corpo_html = corpo_html.replace("{{TABELA}}", jogos.to_html(index=False, justify='center'))
    ...
    # Adiciona imagem no corpo do e-mail
